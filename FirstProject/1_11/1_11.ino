...
// ตัวอักษรสำหรับ "START" และ "SCORE"
byte S[8] = { B00000000,
              B00111110,
              B01000000,
              B00111100,
...
void setup() {
  mySerial.begin(4800);  // เปิดการสื่อสารกับ Arduino ตัวที่ 1
  Serial.begin(4800);    // เปิด Serial Monitor
...
void loop() {
  if (mySerial.available()) {
    ...
    Serial.println("ข้อมูลที่รับ: " + receivedData);  // แสดงข้อมูลที่รับใน Serial Monitor

    if (receivedData.startsWith("IDEL")) {
      displayScrollingText("IDEL");  // แสดงคำว่า START
    } else if (receivedData.startsWith("START")) {
      clearAllDisplays();
    } else if (receivedData.startsWith("POS:")) {
      playerY = receivedData.substring(4).toInt();  // อ่านตำแหน่งแกน Y ของผู้เล่น
      runGame();                                    // แสดงตำแหน่งผู้เล่นบน Dot Matrix
    } else if (receivedData.startsWith("SCORE:")) {
      int score = receivedData.substring(6).toInt();
      displayScrollingScoreWithNumber(score);  // แสดงคะแนน
    }
  }
}
...
void runGame() {
  clearPlayer();
  setLed(playerX, playerY, true);  // แสดงตำแหน่งผู้เล่น
}
...
void displayScrollingScoreWithNumber(int score) {
